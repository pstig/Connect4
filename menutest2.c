...
int main()
{
	...
	char *requests[] = { "Gamemode:", "(1 or 2)", "Difficulty:", "(0-4)", "Number of Rows:", "(Positive Interger)", "Number of Columns:", "(Positive Interger)" };
	int ch;
	...
	new_popup(24, 80, (LINES-25)/2, (COLS-81)/2, buttons, 2, requests, 8); //24,80
	refresh();
	wrefresh(win_body);
	wrefresh(win_form);
	wrefresh(win_menu);
